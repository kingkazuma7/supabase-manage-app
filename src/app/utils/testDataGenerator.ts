...
import { addHours, subDays, subMonths, format, setHours, setMinutes, startOfMonth, addDays } from 'date-fns';
  ...
  switch (pattern) {
    case '休憩付き通常勤務':
      ...
      testData.push({
        ...
        original_clock_in: format(setHours(setMinutes(now, 0), 10), 'yyyy-MM-dd HH:mm:ss'),
        original_clock_out: format(setHours(setMinutes(now, 0), 19), 'yyyy-MM-dd HH:mm:ss')
      });
      break;
    ...
    case '夜勤休憩付き':
      ...
      testData.push({
        ...
        original_clock_in: format(setHours(setMinutes(now, 0), 20), 'yyyy-MM-dd HH:mm:ss'),
        original_clock_out: format(setHours(setMinutes(addHours(now, 24), 0), 5), 'yyyy-MM-dd HH:mm:ss')
      });
      break;
    ...
    case '日付跨ぎ休憩付き':
      ...
      testData.push({
        ...
        break_end: format(setHours(setMinutes(now, 0), 1), 'HH:mm'),
        original_clock_in: format(setHours(setMinutes(now, 0), 22), 'yyyy-MM-dd HH:mm:ss'),
        original_clock_out: format(setHours(setMinutes(addHours(now, 24), 0), 3), 'yyyy-MM-dd HH:mm:ss')
      });
      break;
    ...
    case '複数日':
      ...
      for (let i = 0; i < 3; i++) {
        ...
        testData.push({
          ...
          original_clock_in: format(setHours(setMinutes(date, 0), 9), 'yyyy-MM-dd HH:mm:ss'),
          original_clock_out: format(setHours(setMinutes(date, 0), 18), 'yyyy-MM-dd HH:mm:ss')
        });
      }
      break;
    ...
    case '3ヶ月分':
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const daysInMonth = Array.from({ length: 30 }, (_, k) => addDays(monthStart, k));
