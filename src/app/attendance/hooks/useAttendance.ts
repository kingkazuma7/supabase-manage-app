...
export const useAttendance = (staffId: string | null) => {
  ...
  const createDailyWorkTime = (
    record: Pick<
      AttendanceRecord,
      "originalClockIn" | "originalClockOut" | "breakStart" | "breakEnd"
    >,
    staffName: string,
  ): WorkTime => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (attendanceData) {
        ...
        // 本日の出退勤完了チェック
        const todayCompleted = attendanceData.some(
          (record) =>
            record.clock_in &&
            new Date(record.clock_in).toISOString().split("T")[0] ===
              todayIso &&
            record.clock_out,
        );
        ...
        const formattedRecords = attendanceData
          .filter((record) => {
            ...
            const recordClockOut = record.clock_out
              ? new Date(record.clock_out)
              : null;
            return (
              (recordClockIn.getFullYear() === targetYear &&
                recordClockIn.getMonth() === targetMonth) ||
              (recordClockOut &&
                recordClockOut.getFullYear() === targetYear &&
                recordClockOut.getMonth() === targetMonth &&
                recordClockIn.getMonth() !== targetMonth)
            );
          })
          .map((record) => {
            ...
            return {
              ...
              clockIn: formatTimeString(recordClockIn),
              clockOut: recordClockOut
                ? formatTimeString(recordClockOut)
                : null,
        ...
        const currentStatus: AttendanceStatus = {
          ...
          lastClockIn:
            lastUnclockedOut?.clock_in || lastClockedOut?.clock_in || null,
          lastClockOut: lastUnclockedOut
            ? null
            : lastClockedOut?.clock_out || null,
          status: lastUnclockedOut
            ? "勤務中"
            : lastClockedOut
              ? "退勤済み"
              : null,
          message: lastUnclockedOut
            ? "勤務中"
            : lastClockedOut
              ? "退勤済み"
              : null,
          isOnBreak: !!(
            lastUnclockedOut?.break_start && !lastUnclockedOut?.break_end
          ),
      ...
      setError(
        err instanceof Error ? err.message : ATTENDANCE_ERRORS.FETCH_ERROR,
      );
    }
  ...
  const handleAttendance = async (type: "出勤" | "退勤") => {
    try {
      ...
      const { data: allRecentRecords, error: recentRecordsError } =
        await supabase
          .from("attendance")
          .select("*")
      ...
      if (type === "出勤") {
        ...
        if (
          ...
          new Date(latestRecord.clock_in).toISOString().split("T")[0] ===
            todayIso
        ) {
  ...
  const handleBreak = async (type: "休憩開始" | "休憩終了") => {
    try {
      ...
      setError(
        error instanceof Error ? error.message : ATTENDANCE_ERRORS.FETCH_ERROR,
      );
    }
  };
